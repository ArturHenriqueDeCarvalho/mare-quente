...
function Catalog() {
  ...
  return (
    <div className="page-container">
      {/* Hero */}
      <div className="hero-banner" style={{ background: 'var(--grad-sunset)', borderRadius: 'var(--r-xl)', padding: '44px 48px', color: '#fff', margin: '24px 0 32px', position: 'relative', overflow: 'hidden' }}>
        <div className="mobile-hidden" style={{ position: 'absolute', width: 320, height: 320, borderRadius: '50%', background: 'rgba(255,255,255,.13)', top: -100, right: -40 }} />
      ...
      {/* Filtros */}
      <div className="between row wrap" style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 26, fontFamily: 'var(--font-display)' }}>{titles[filter]}</h2>
        <div className="row filters-nav" style={{ gap: 8 }}>
          {filtersList.map(([f, l]) => (
